...
notes. Treat every change accordingly. This file mirrors `AGENTS.md`
(the tool-agnostic source of truth) in Gemini-specific framing — if the
...
conflicts or staleness for whoever you're working with.
...
---
*This file is tracked in the repo as a shared starting point, but is
listed in `.gitignore` — local edits here won't be picked up by `git
status` for new changes. Update `AGENTS.md` instead if the shared
instructions need to change; treat this file as a synced copy, not the
source of truth.*
