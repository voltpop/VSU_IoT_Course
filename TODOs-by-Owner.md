...
8. **Aug 14** — Budget revision *(shared: Builder Tech + VoltPop + Explay + Dr. Shawn M. Nicholson LLC — compensation for all appears in the budget; ties to several Builder Tech items below)*.
9. **Aug 17** — Program revision *(scope unclear — likely `Course-Curriculum.md`'s schedule/program document; shared: Builder Tech + VoltPop + Explay; confirm with Javon)*.
10. **Aug 19** — Pitch rehearsal *(shared: VoltPop + Explay + Builder Tech)*.
11. **Aug 21** — Client presentation created & delivered — the milestone itself *(shared: all)*.
...
3. **[Due mid-Aug 2026]** Follow through on Virginia Gateway Region (VGR) research/outreach *(shared: Dr. Shawn M. Nicholson LLC)* — fill in `Stakeholder-Notes.md`'s VGR profile as this progresses (Admin-Business-Legal.md §8).
4. **[Due mid-Aug 2026]** Follow up with Dominion Energy, who already expressed interest *(shared: Dr. Shawn M. Nicholson LLC)* — strong entry point: James Beazley (VGR Secretary, Dominion Energy) is a personal contact of Dr. Nicholson's — fill in `Stakeholder-Notes.md`'s Dominion profile as this progresses (Admin-Business-Legal.md §8).
5. **Aug 7** — Frame any stakeholder conversations as exploratory/preliminary, not firm commitments on VSU's behalf, given outreach is happening before formal VSU advancement buy-in — see the framing guidance at the top of `Stakeholder-Notes.md` (Admin-Business-Legal.md §8).
6. **Aug 21** — Build the actual tailored pitch deck/press kit from `Stakeholder-Notes.md`'s per-stakeholder research (once filled in) and lock final budget/ask figures and logos/photos *(shared: Explay + Builder Tech, since it feeds the joint client presentation)* — see `Stakeholder-Notes.md` and `Press-Kit-Content.md`.
7. *(per VoltPop, likely deferred until after stakeholder conversations bring in upfront funding — not urgent despite the "before Week 1" framing)* Confirm own background-check (Checkr) timeline before Week 1 (Admin-Business-Legal.md §9).

## Explay

1. *(feeds VoltPop's Aug 21 press-kit deliverable)* Provide Explay's longer "about" boilerplate for the press kit — currently a placeholder *(shared: VoltPop)* (Press-Kit-Content.md).
2. *(feeds VoltPop's Aug 21 press-kit deliverable)* Provide Emanuel Perez's personal "about me" blurb and headshot for the press kit *(shared: VoltPop)* (Press-Kit-Content.md).
...
4. Design Week 9's shrunken security/APIs content around the 4D lens, since Explay leads that week — currently unstructured *(shared: VoltPop)* (Course-Curriculum.md §7).
5. Help resolve Week 9's detailed division of labor with VoltPop (product-iteration vs. technical-debugging/integration/security split) — still unresolved despite Explay's lead/support assignments being confirmed *(shared: VoltPop)* (Course-Curriculum.md §6).
